...
namespace Nfh.Dal.Xml.Converters
{
    internal class Converter : IConverter
    {
        ...
        {
            if (from == typeof(List<XmlLevelFloor>) && to == typeof(List<Domain.Models.InGame.Wall>))
            {
                return converters.Values.FirstOrDefault(c => c.GetType().Name.Contains("WallsConverter"));
            }
            if (from == typeof(List<XmlLevelFloor>) && to == typeof(List<Domain.Models.InGame.Floor>))
            {
        ...
        {
            if (to == typeof(List<XmlLevelFloor>) && from == typeof(List<Domain.Models.InGame.Wall>))
            {
                return converters.Values.FirstOrDefault(c => c.GetType().Name.Contains("WallsConverter"));
            }
            if (to == typeof(List<XmlLevelFloor>) && from == typeof(List<Domain.Models.InGame.Floor>))
            {
                return converters.Values.FirstOrDefault(c => c.GetType().Name.Contains("FloorsConverter"));
            }
            return null;
